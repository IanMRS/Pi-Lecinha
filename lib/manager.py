...
class GenericManager(Frame):
    def __init__(self, crud, frame):
        ...
        self.inputs.grid_columnconfigure(0, weight=1)
        self.inputs.grid_rowconfigure(0, weight=1)
    ...
    def on_enter(self, event=None):
        (self.update_button_pressed if "" not in self.get() else self.search_button_pressed)()

    def on_control_enter(self, event=None):
        (self.insert_button_pressed if self.get()[0] == ""else self.update_button_pressed)()
    ...
    def insert_button_pressed(self, event=None):
        self.crud.insert(self.get())
        self.clear_button_pressed()

    def update_button_pressed(self, event=None):
        condition = f"id = {self.get()[0]}"
        self.crud.update(self.get(), condition)
        self.clear_button_pressed()

    def search_button_pressed(self, event=None):
        search_result = self.crud.search(self.get())
        self.data_table.refresh_table(search_result)
        self.inputs.clear()
    ...
    def delete_confirmed(self):
        self.crud.delete(f"id = {self.get()[0]}")
        self.clear_button_pressed()
        self.popup.destroy()
